...
class Shark : public PlayerCreature {
public:
    Shark(PlayerCreature& player, std::shared_ptr<GameSprite> sprite)
        : PlayerCreature(player) 
    {
        setCollisionRadius(80);         
        changeSpeed(player.getSpeed() + 10); 
        m_sprite = sprite;              
    ...
    void draw() const override {
        ofLogVerbose() << "Shark at (" << m_x << ", " << m_y << ")!" << std::endl;
        if (m_sprite) m_sprite->draw(m_x, m_y);
    }
};

class Fish : public NPCreature{
    public:
        Fish(float x, float y, int speed, std::shared_ptr<GameSprite> sprite);
        void move() override;
        void draw() const override;
};
...
class AquariumSpriteManager {
    ...
    private:
        ...
        std::shared_ptr<GameSprite> m_fish;
};
...
public:
    ...
    std::shared_ptr<AquariumSpriteManager> GetSpriteManager() const {
...
class Level_0 : public AquariumLevel  {
    public:
        Level_0(int levelNumber, int targetScore): AquariumLevel(levelNumber, targetScore){
            ...
            this->m_levelPopulation.push_back(std::make_shared<AquariumLevelPopulationNode>(AquariumCreatureType::Fish, 5));
